...
function getPolynom(...k) {
  const [k0 = 0, k1 = 0, k2 = 0] = [...k].reverse();
  return (x) => k2 * (x ** 2) + k1 * x + k0;
}
...
function memoize(fn) {
  ...
  return (n) => {
    ...
    const args = (isObj) ? Array.prototype.slice.call() : n;
    const key = (isObj) ? JSON.stringify(args) : n;
    if (!(key in memo)) {
